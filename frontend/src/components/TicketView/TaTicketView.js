...
const TaTicketView = ({ ticketId, onClose }) => {
    ...
    const onOpenTicket = () => {
        navigate(`/taticketinfo?ticket=${ticketId}`)
    }

    const fetchTicketDetails = async () => {
        try {
            const token = Cookies.get("token");
            const response = await fetch(`${baseURL}/api/tatickets/${ticketId}`, {
                method: "GET",
                headers: {
